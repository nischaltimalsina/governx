...
import { AssignAssetOwnerDTO, AssetDTO } from '../dtos/asset_dtos'

/**
 * Use case for assigning an owner to an asset
 */
export class AssignAssetOwnerUseCase {
  ...
  public async execute(
    assetId: string,
    request: AssignAssetOwnerDTO,
    userId: string
  ): Promise<Result<AssetDTO, Error>> {
    // Call domain service to assign asset owner
    const assetResult = await this.assetManagementService.assignAssetOwner(
      assetId,
      request.ownerId,
      request.ownerName,
