...
export class FrameworkController {
  constructor(
    ...
    private updateFrameworkUseCase: UpdateFrameworkUseCase
  ) {}
   ...
   */
  public updateFramework = async (req: Request, res: Response): Promise<void> => {
    try {
      // Ensure userId is available from auth middleware
      if (!req.userId) {
        res.status(401).json({
      ...
      const { id } = req.params
      const result = await this.updateFrameworkUseCase.execute(
        id,
        {
